...
[![Theorems](https://img.shields.io/badge/Theorems_proven-180개-success?style=flat-square)](./README.md)
...
| "Regularization이 일반화를 돕습니다" | **Uniform stability** $\beta$ 정의, Ridge가 $\beta = O(1/\lambda n)$ 임을 **strong convexity로** 증명, **Hardt et al. 2016**의 SGD stability $\beta \leq O(\eta T)$로 "early stopping = implicit regularization" |
...
| [03. VC 차원 계산 — 사각형·원·다각형](./ch4-vc-dimension/03-geometric-shapes-vc.md) | **축정렬 직사각형 VC $= 4$**, **임의 방향 직사각형 VC $= 5$**, **원(center+radius) VC $= 3$**, convex polygon($k$-gon)의 VC $= 2k+1$, 기하학적 arg |
...
| [07. VC 경계의 한계와 실전의 의미](./ch4-vc-dimension/07-vc-limits.md) | 신경망의 **VC $= O(W^2 \log W)$**가 엄청나게 크지만 일반화 잘 되는 **"paradox"**, VC bound가 실전에서 **vacuous**(의미없을 정도로 큼), **Generalization Theory 레포**의 필요성 |
...
| [04. SGD의 Stability (Hardt et al. 2016)](./ch6-stability/04-sgd-stability.md) | 유한 단계 SGD가 **$\beta \leq O(\eta T)$**로 stable, **non-expansive + $\eta$-small-step 분석**, 왜 "적게 훈련"이 **implicit regularization**인가, deep learning의 early stopping 정당화 |
...
이 레포에서 **완전한 증명**을 제공하는 대표 정리 모음입니다. 각 챕터의 문서에서 $\square$로 종결되는 엄밀한 증명을 확인할 수 있습니다. (전체 180+ 정리 중 핵심만 발췌)
...
> 💡 **챕터별 총 정리 수**: Ch1(18) · Ch2(24) · Ch3(22) · Ch4(37) · Ch5(32) · Ch6(21) · Ch7(26) — 합계 **180개 정리 + 증명**, 약 **14,000+ 라인** 분량.
